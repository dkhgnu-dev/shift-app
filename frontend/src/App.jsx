                    ...
                    <div className="tab-content active">
                        ...
                        <div style={{display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', flexWrap: 'wrap'}}>
                            ...
                            <button
                                ...
                                // Take2(Dex P4差戻しP1): 全画面表示中は`.matrix-glass-card`が
                                // position:fixed; z-index:9999で画面全体を覆うため、通常の
                                // 静的配置のままだとスタンプ開始ボタンが隠れて操作できなくなる。
                                // 全画面表示の閉じるボタンと同じ手法(position:fixed + より高い
                                // z-index)で、全画面中も独立した操作可能なレイヤーとして浮かせる。
                                style={isFullScreen ? { position: 'fixed', bottom: '16px', left: '16px', zIndex: 10000, boxShadow: '0 4px 12px rgba(0,0,0,0.35)' } : undefined}
                            >
                        ...
                        {/* Cycle12 5: スタンプパレット。表の固定列・ズーム・左右フロートボタンとは
                            ...
                            Take2(Dex P4差戻しP1): 全画面表示中も同じくposition:fixedで浮かせ、
                            トグルボタンの上に重ねて配置する(連続スタンプ・筆変更・終了を維持)。 */}
                        {stampModeEnabled && (
                            <div
                                className="stamp-palette glass-card"
                                role="group"
                                aria-label="スタンプの筆を選ぶ"
                                style={isFullScreen ? { position: 'fixed', bottom: '70px', left: '16px', right: '16px', zIndex: 10000, maxHeight: '40vh', overflowY: 'auto', boxShadow: '0 4px 16px rgba(0,0,0,0.35)' } : undefined}
                            >
                            ...
                            <>
                                ...
                                <div className="glass-card matrix-glass-card" style={{padding: '16px'}}>
                                    <div className="matrix-scroll-wrapper">
                                        ...
                                        <table className={isNameColumnCollapsed ? 'name-col-collapsed' : ''} style={{ zoom: isMobileView ? (isFullScreen ? '35%' : '100%') : `${zoomLevel}%` }}>
                                            <thead>
                                                <tr>
