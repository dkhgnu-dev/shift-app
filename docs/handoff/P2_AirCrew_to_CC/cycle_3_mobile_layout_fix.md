...
1. **基本レイアウト修正:** `.app-container` クラスにPC用の `display: flex; width: 100%; min-height: 100vh;` を必ず定義し、`@media (max-width: 768px)` の中だけで `flex-direction: column;` になるようにする。（PC版が縦積みにならないようにするため）
...
4. **iOSズーム防止:** スマホ幅の際、`.form-control`, `.time-picker-input`, `.time-picker-value` に加え、すべての `input, select, textarea` の `font-size` が `16px` 以上になるよう設定し、iPhone等でのタップ時自動ズームを防ぐ。
...
### 3. バージョン表記の更新
表示バージョンを `v4.16` から `v4.17` へ更新してください。
